...
class Synthesizer(Organism):
    ...
    Generates: GFF3, GenBank, JSON summary, protein FASTA,
    and the audit trail.
    """
    ...
    async def grow(self, nutrient: Nutrient) -> Nutrient | None:
        ...
        self._write_genbank(genome, gbk_path)
    ...
    def _write_genbank(self, genome: Genome, path: Path) -> None:
        """Write GenBank flat file format output."""
        with open(path, "w") as fh:
            for contig in genome.contigs:
                # Header
                fh.write(
